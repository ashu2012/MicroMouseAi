"""###################################################
    FLOOD FILL ALGORITHM FOR A MICROMOUSE MAZE
    Kansas State University Robotics Competition Team
 
    Coded by: Richard Habeeb, Jarrod Krebs
    Rev 1
####################################################"""
 
print "############################BEGIN MICROMOUSE TEST#############################"
...
PosR = 15 #Row position
PosC = 0 #Column position
direction = "N" #robots rotation (heading) N, E, S, W
...
mazeWalls = [0]*16 #maze wall storage NESW
for j in range(0,16): #one way of creating nested list
    mazeWalls[j] = [0]*16
    for k in range(0,16): 
        mazeWalls[j][k] = [0]*4
 
#Initializing the depth array 16x16 signed int
mazeDepth = [0]*16 #another way of creating nested list (more efficient)
